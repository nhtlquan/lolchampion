
package vn.lequan.lienminhsamsoi.models.VideoChampion;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

public class Title___ {

    @SerializedName("cs_cz")
    @Expose
    private String csCz;
    @SerializedName("de_de")
    @Expose
    private String deDe;
    @SerializedName("el_gr")
    @Expose
    private String elGr;
    @SerializedName("en_au")
    @Expose
    private String enAu;
    @SerializedName("en_gb")
    @Expose
    private String enGb;
    @SerializedName("en_ph")
    @Expose
    private String enPh;
    @SerializedName("en_pl")
    @Expose
    private String enPl;
    @SerializedName("en_sg")
    @Expose
    private String enSg;
    @SerializedName("en_us")
    @Expose
    private String enUs;
    @SerializedName("es_ar")
    @Expose
    private String esAr;
    @SerializedName("es_es")
    @Expose
    private String esEs;
    @SerializedName("es_mx")
    @Expose
    private String esMx;
    @SerializedName("fr_fr")
    @Expose
    private String frFr;
    @SerializedName("hu_hu")
    @Expose
    private String huHu;
    @SerializedName("id_id")
    @Expose
    private String idId;
    @SerializedName("it_it")
    @Expose
    private String itIt;
    @SerializedName("ja_jp")
    @Expose
    private String jaJp;
    @SerializedName("ko_kr")
    @Expose
    private String koKr;
    @SerializedName("ms_my")
    @Expose
    private String msMy;
    @SerializedName("pl_pl")
    @Expose
    private String plPl;
    @SerializedName("pt_br")
    @Expose
    private String ptBr;
    @SerializedName("ro_ro")
    @Expose
    private String roRo;
    @SerializedName("ru_ru")
    @Expose
    private String ruRu;
    @SerializedName("th_th")
    @Expose
    private String thTh;
    @SerializedName("tr_tr")
    @Expose
    private String trTr;
    @SerializedName("vn_vn")
    @Expose
    private String vnVn;
    @SerializedName("zh_cn")
    @Expose
    private String zhCn;
    @SerializedName("zh_tw")
    @Expose
    private String zhTw;

    public String getCsCz() {
        return csCz;
    }

    public void setCsCz(String csCz) {
        this.csCz = csCz;
    }

    public String getDeDe() {
        return deDe;
    }

    public void setDeDe(String deDe) {
        this.deDe = deDe;
    }

    public String getElGr() {
        return elGr;
    }

    public void setElGr(String elGr) {
        this.elGr = elGr;
    }

    public String getEnAu() {
        return enAu;
    }

    public void setEnAu(String enAu) {
        this.enAu = enAu;
    }

    public String getEnGb() {
        return enGb;
    }

    public void setEnGb(String enGb) {
        this.enGb = enGb;
    }

    public String getEnPh() {
        return enPh;
    }

    public void setEnPh(String enPh) {
        this.enPh = enPh;
    }

    public String getEnPl() {
        return enPl;
    }

    public void setEnPl(String enPl) {
        this.enPl = enPl;
    }

    public String getEnSg() {
        return enSg;
    }

    public void setEnSg(String enSg) {
        this.enSg = enSg;
    }

    public String getEnUs() {
        return enUs;
    }

    public void setEnUs(String enUs) {
        this.enUs = enUs;
    }

    public String getEsAr() {
        return esAr;
    }

    public void setEsAr(String esAr) {
        this.esAr = esAr;
    }

    public String getEsEs() {
        return esEs;
    }

    public void setEsEs(String esEs) {
        this.esEs = esEs;
    }

    public String getEsMx() {
        return esMx;
    }

    public void setEsMx(String esMx) {
        this.esMx = esMx;
    }

    public String getFrFr() {
        return frFr;
    }

    public void setFrFr(String frFr) {
        this.frFr = frFr;
    }

    public String getHuHu() {
        return huHu;
    }

    public void setHuHu(String huHu) {
        this.huHu = huHu;
    }

    public String getIdId() {
        return idId;
    }

    public void setIdId(String idId) {
        this.idId = idId;
    }

    public String getItIt() {
        return itIt;
    }

    public void setItIt(String itIt) {
        this.itIt = itIt;
    }

    public String getJaJp() {
        return jaJp;
    }

    public void setJaJp(String jaJp) {
        this.jaJp = jaJp;
    }

    public String getKoKr() {
        return koKr;
    }

    public void setKoKr(String koKr) {
        this.koKr = koKr;
    }

    public String getMsMy() {
        return msMy;
    }

    public void setMsMy(String msMy) {
        this.msMy = msMy;
    }

    public String getPlPl() {
        return plPl;
    }

    public void setPlPl(String plPl) {
        this.plPl = plPl;
    }

    public String getPtBr() {
        return ptBr;
    }

    public void setPtBr(String ptBr) {
        this.ptBr = ptBr;
    }

    public String getRoRo() {
        return roRo;
    }

    public void setRoRo(String roRo) {
        this.roRo = roRo;
    }

    public String getRuRu() {
        return ruRu;
    }

    public void setRuRu(String ruRu) {
        this.ruRu = ruRu;
    }

    public String getThTh() {
        return thTh;
    }

    public void setThTh(String thTh) {
        this.thTh = thTh;
    }

    public String getTrTr() {
        return trTr;
    }

    public void setTrTr(String trTr) {
        this.trTr = trTr;
    }

    public String getVnVn() {
        return vnVn;
    }

    public void setVnVn(String vnVn) {
        this.vnVn = vnVn;
    }

    public String getZhCn() {
        return zhCn;
    }

    public void setZhCn(String zhCn) {
        this.zhCn = zhCn;
    }

    public String getZhTw() {
        return zhTw;
    }

    public void setZhTw(String zhTw) {
        this.zhTw = zhTw;
    }

}
