...
public class TouchImageView extends ImageView {
    ...
    class C08371 implements OnTouchListener {
        C08371() {
        }
    ...
    private void sharedConstructing(Context context) {
        ...
        this.f56m = new float[9];
        setImageMatrix(this.matrix);
        setScaleType(ScaleType.MATRIX);
        setOnTouchListener(new C08371());
    }
    ...
    void fixTrans() {
        this.matrix.getValues(this.f56m);
        float transX = this.f56m[2];
        float transY = this.f56m[5];
